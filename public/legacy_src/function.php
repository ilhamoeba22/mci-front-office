...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'mail.bprsmci.net';  // Masukkan Server SMTP
  $mail->Port 		  = 587;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'tls';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'adminlpk@bprsmci.net';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= 'B0w0s4j4##';        						// Masukkan Password dari Email tsb
  $default_email_from       = 'adminlpk@bprsmci.net';        // Masukkan default from pada email
